 ...
 */
angular.module('dashyAngular').controller('delete_user', function ($scope, $http) {
    ...
    $scope.init = function () {
        $http({
            ...
            $scope.selected = response.data.result[0].userid;
	...
	if (!$scope.formsinglesublitControleFlag) {
				...
				$http({
					method: 'DELETE',
					url: 'http://localhost:8084/SmileClass/useraccounts/' + $scope.selected
				}).then(function successCallback(response) {
