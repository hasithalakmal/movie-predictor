 ...
 */
angular.module('dashyAngular').controller('view_half_paid_students', function ($scope, $http, $sce) {
    ...
    $scope.init = function () {
        ...
        $http({
            method: 'GET',
            url: 'http://localhost:8084/SmileClass/class-all-halfcard-students'
        }).then(function successCallback(response) {
	...
	$scope.submit = function () {
         $http({
            method: 'GET',
            url: 'http://localhost:8084/SmileClass/class-all-halfcard-students/'+ $scope.selected
        }).then(function successCallback(response) {
            $scope.classess = response.data.result;
