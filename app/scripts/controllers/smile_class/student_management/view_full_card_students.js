 ...
 */
angular.module('dashyAngular').controller('view_full_card_students', function ($scope, $http, $sce) {
    ...
    $scope.init = function () {
        ...
        $http({
            method: 'GET',
            url: 'http://localhost:8084/SmileClass/class-all-fullcard-students'
        }).then(function successCallback(response) {
	...
	$scope.submit = function () {
         $http({
            method: 'GET',
            url: 'http://localhost:8084/SmileClass/class-all-fullcard-students/'+ $scope.selected
        }).then(function successCallback(response) {
            $scope.classess = response.data.result;
