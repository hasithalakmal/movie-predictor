 ...
 */
angular.module('dashyAngular').controller('student_reassign_to_class', function ($scope, $http) {
            ...
            if (!$scope.formsinglesublitControleFlag1) {
                ...
                $http({
                    ...
                    if (response.data.result) {
                        ...
                        $http({
                            method: 'GET',
                            url: 'http://localhost:8084/SmileClass/get-students-remove-classes/' + $scope.addmision_number
                        }).then(function successCallback(response3) {
                            $scope.learn_classes_select = response3.data.result;
							console.log($scope.learn_classes_select);
	...
	$scope.submit2 = function(){
		if (!$scope.formsinglesublitControleFlag2) {
				...
				$http({
                            method: 'GET',
                            url: 'http://localhost:8084/SmileClass/re-assign-student-classes/' + $scope.selected
                        }).then(function successCallback(response3) {
							$scope.addmision_number = null;
							$scope.selectClass = false;
							swal('Success!', 'Student Re-Assigned To Class!', 'success');
							$scope.formsinglesublitControleFlag2 = false;
                        }, function errorCallback(response) {
                            swal('Error!', 'Something Wrong!', 'error');
							$scope.formsinglesublitControleFlag2 = false;
